...
const retryableErrorMessage =
  "This version did not regenerate cleanly. Keep the current plan, then try again with fewer changes in one request.";
...
export function RegenerateBox({
  ...
  async function submitRegeneration() {
    if (!hasTripRequest) {
      ...
      setErrorMessage(
        "The original trip request is missing. Please return to /plan and generate again before asking for a safe rewrite.",
      );
    ...
    if (!trimmedRequest) {
      setStatusDraftVersion(externalDraftVersion);
      setErrorMessage("Write one clear change request first.");
      setSuccessMessage(undefined);
      setAppliedChanges([]);
      return;
    }
    ...
    if (trimmedRequest.length > TRIP_INPUT_LIMITS.modificationRequest) {
      setStatusDraftVersion(externalDraftVersion);
      setErrorMessage(
        `Keep the edit request within ${TRIP_INPUT_LIMITS.modificationRequest} characters.`,
      );
      setSuccessMessage(undefined);
      setAppliedChanges([]);
      return;
    ...
    try {
      ...
      if (!parsed.success) {
        throw new Error(
          "The returned content was incomplete. Please retry with a smaller or clearer edit request.",
        );
      }
      ...
      setSuccessMessage("A refreshed version has been prepared from your latest instructions.");
      onModificationRequestChange("");
    } catch (error) {
      setStatusDraftVersion(externalDraftVersion);
      setErrorMessage(
    ...
    >
      ...
      <div className="relative z-[1]">
        ...
        <h2 id="regenerate-title" className="mt-2 text-lg font-semibold sm:text-xl">
          Tell the travel assistant what should change
        </h2>
        <p className="mt-2 break-words text-sm leading-6 text-[var(--ink-muted)]">
          Example: make this day lighter, add a night market, or avoid early starts.
        </p>

        <label className="mt-4 block">
          <span className="sr-only">Regenerate modification request</span>
          <textarea
            ...
            rows={4}
            placeholder="Make this version more relaxed, or tell me exactly which part should change."
        ...
        <div className="mt-2 flex flex-wrap items-center justify-between gap-3 text-xs text-[var(--ink-muted)]">
          <p className="min-w-0 break-words">
            The system keeps the current departure, budget, interests, and this plan as the rewrite base.
          </p>
        ...
        {!hasTripRequest ? (
          <div className="mt-4 rounded-[18px] border-l-2 border-[var(--clay)] bg-[var(--paper-bright)] px-4 py-3 text-sm leading-6 text-[var(--clay-deep)]">
            The original request is incomplete, so safe regeneration is unavailable right now.
            <div className="mt-3">
              <Link
                href="/plan"
        ...
        <div className="mt-4 flex flex-col gap-3 sm:mt-5 sm:flex-row sm:flex-wrap sm:items-center">
          ...
          >
            {isSubmitting ? "Adjusting..." : "Ask AI to adjust"}
          </button>
          ...
          {isSubmitting ? (
            <p className="text-sm text-[var(--ink-muted)]">
              Keeping your preferences and rebuilding a softer version.
            </p>
          ) : null}
        </div>
      </div>
